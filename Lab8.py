import sys
from PySide6.QtWidgets import QApplication, QGraphicsRectItem, QGraphicsScene, QGraphicsView, QPushButton, QHBoxLayout, QWidget, QVBoxLayout, QGraphicsItem
...
class CustomRectItem(QGraphicsRectItem):
    def __init__(self, color, width, height, corner_radius=0, texts=None, text_colors=None, font_sizes=None, parent=None):
        ...
        self.setAcceptHoverEvents(True)  # Дозволяємо обробку подій наведення миші
        self.isSelected = False
        self.setFlag(QGraphicsItem.ItemIsMovable, True)  # Дозволяємо переміщення об'єкту
    ...
    def paint(self, painter, option, widget):
        ...
        for text, color, font_size in zip(self.texts, self.text_colors, self.font_sizes):
            ...
            if 'https://www.github.com/' in text:
    ...
    def hoverEnterEvent(self, event):
        self.setCursor(Qt.PointingHandCursor)  # Змінюємо курсор при наведенні на об'єкт

    def mousePressEvent(self, event):
        self.isSelected = True
        self.update()  # Оновлюємо вигляд об'єкта при натисканні миші
        super().mousePressEvent(event)

    def mouseReleaseEvent(self, event):
        self.isSelected = False
        self.update()  # Оновлюємо вигляд об'єкта при відпусканні миші
