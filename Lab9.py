...
class CustomScene(QGraphicsScene):
    ...
    def __init__(self, colors, positions, rotations, texts, text_colors, font_sizes, corner_radius=None, parent=None):
        ...
        self.timer.timeout.connect(self.animateRotation)
        self.timer.start()
    ...
    def setupScene(self):
        for color, pos, rotation, text, text_color, font_size, radius in zip(self.colors, self.positions, self.rotations, self.texts, self.text_colors, self.font_sizes, self.corner_radius):
            ...
            rect.setPos(*pos)
            rect.setRotation(rotation)  # Устанавливаем переданный угол поворота
            self.addItem(rect)
    ...
    def animateRotation(self):
        for item in self.items():
            if isinstance(item, CustomRectItem):
                item.rotate(1)
                item.setRotation(item.rotationAngle)  # Применяем измененный угол поворота к элементу
